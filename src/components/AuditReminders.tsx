...
const AuditReminders: React.FC = () => {
  ...
  return (
    <div className="p-8">
      <div className="mb-8">
        <h1 className="text-3xl font-bold text-slate-900 mb-2">Audit & Reminders</h1>
        <p className="text-slate-600">Stay on top of your compliance obligations and audit schedules</p>
      </div>
      ...
      {selectedView === 'upcoming' && (
        <div className="space-y-6">
          {upcomingAudits.map((audit) => (
            <div key={audit.id} className="bg-white rounded-lg p-6 shadow-sm border border-slate-200">
              <div className="flex items-start justify-between mb-4">
                <div className="flex items-start space-x-4">
                  ...
                  <div>
                    <h3 className="text-xl font-semibold text-slate-900">{audit.title}</h3>
                    <p className="text-slate-600">{audit.framework} • {audit.auditor}</p>
              ...
              <div className="border-t border-slate-200 pt-4">
                <h4 className="font-medium text-slate-900 mb-3">Preparation Tasks</h4>
      ...
      {selectedView === 'reminders' && (
        <div className="space-y-4">
          {reminders.map((reminder) => (
            <div key={reminder.id} className="bg-white rounded-lg p-4 shadow-sm border border-slate-200 flex items-center justify-between">
              <div className="flex items-center space-x-4">
                ...
                <div>
                  ...
                  <p className="text-slate-500 text-xs mt-1">Due: {reminder.date}</p>
                </div>
              </div>
              <div className="flex items-center space-x-2">
      ...
      {selectedView === 'history' && (
        <div className="space-y-4">
          {pastAudits.map((audit) => (
            <div key={audit.id} className="bg-white rounded-lg p-4 shadow-sm border border-slate-200">
              <div className="flex items-center justify-between">
                <div className="flex items-center space-x-4">
                  ...
                  <div>
                    ...
                    <p className="text-slate-600 text-sm">{audit.framework} • {audit.auditor}</p>
                ...
                <div className="text-right">
                  <div className="flex items-center space-x-2 mb-1">
                    <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                      audit.result === 'Passed' ? 'bg-green-100 text-green-800' : 'bg-red-100 text-red-800'
                    }`}>
                      {audit.result}
                    </span>
                    <span className="text-lg font-bold text-slate-900">{audit.score}%</span>
                  </div>
                  <p className="text-slate-500 text-xs">{audit.findings} findings</p>
                </div>
              </div>
            </div>
          ))}
